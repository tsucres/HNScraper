...
public class HNScraper {
     ...
     */
    private func downloadHtmlPage(urlString: String, cookies: [HTTPCookie] = [], completion: @escaping ((String?, HNScraperError?) -> Void), tries: Int = 3) {
        RessourceFetcher.shared.fetchData(urlString: urlString, cookies: cookies, completion: {(data, error) -> Void in
            ...
            if decodedHtml.error == .serverUnreachable &&
                decodedHtml.html?.contains("<td>\n          Sorry, we\'re not able to serve your requests this quickly.\n        </td>") ?? false &&
                tries > 0 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(rndInt), execute: { [weak self] in
                    self?.downloadHtmlPage(urlString: urlString, cookies: cookies, completion: completion, tries: tries-1)
                })
            } else {
    ...
    private func submitSubmissionForm(WithData bodyData: Data, pathComponent: String, text:String?, completion: @escaping (HNScraperError?) -> Void) {
        ...
        // For testing purpose: comment the previous block qnd uncomment the following line.
        //completion(nil)
